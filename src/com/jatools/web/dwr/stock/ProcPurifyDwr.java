 ...
 * Created 2011-11-24
 */
public class ProcPurifyDwr extends BaseChangeDwr {
	private static Logger logger = Logger.getLogger(ProcPurifyDwr.class);
	...
	public String saveOrUpdatePurify(ProcChangeHead procChangeHead, List<ProcChangeLine> oldLines, String ids, HttpServletRequest req) {
		try{
			if(StringUtil.isEmpty(procChangeHead.getBillid())){
				...
				String billNo = commonManager.getBillno(GlobalConstant.BILL_CODE_TICHUN);
	...
	public String saveAndCheckPurify(ProcChangeHead procChangeHead, List<ProcChangeLine> oldLines, String ids, HttpServletRequest req) {
		try{
			if(StringUtil.isEmpty(procChangeHead.getBillid())){
				procChangeHead.setCreateDate(DateUtil.getCurrentDate18());
				procChangeHead.setCreateId(CommonUtil.getSessionUserId(req));
