function loadQ(){
	//SbraCoefficientDwr.getQualityByItemClassId(jQuery("#itemClassId").val(),function(data){
...
function saveSbraCoefficient(){
	...
	if(Validator.Validate($("frm"),3)){
		confirm("确定保存?", function(){
    		...
    		SbraCoefficientDwr.saveOrUpdateSbraCoefficient(headObj, function(data){
    			var m = data.split("_");
    			if(m[0]=="2"){
    				alert(m[1]);
    			}else{
    				alert(m[1], function(){
    					window.location = "sbraCoefficient.vm";
    				});
    			}
    		});
    	});
