function saveCleanGrade(){
	if(Validator.Validate($("frm"),3)){
		confirm("确定保存?", function(){
    		...
    		CleanGradeDwr.saveOrUpdateCleanGrade(headObj, function(data){
    			var m = data.split("_");
    			if(m[0]=="2"){
    				alert(m[1]);
    			}else{
...
function deleteCleanGrade(){
	...
	confirm("确定删除?", function(){
		window.location = "cleanGrade.vm?user_action=deleteCleanGrade&id=" + id;
	});
}