...
function resetSum(){
	var sumCount=sumWeight=sumGrains=saleMoney=0;
	...
	for(var i=0;i<$("tbl").rows.length;i++){
		...
		saleMoney=floatAdd(saleMoney, $n("#posCost")[i].value||0);
		jQuery("#stockId").attr("disabled",true);
	}
	//jQuery("#saleMoney").val(saleMoney);
	...
	jQuery("#sumCost").val(saleMoney.toFixed(6));
}
