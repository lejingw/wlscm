...
function addRow(line){
	...
	var hidden="<input type='hidden' name=\"#lineId\" value=\"\">" +
		...
		"<input type='hidden' name=\"#colorId\"  value=\""+(line.colorId||0)+"\">" + 
		"<input type='hidden' name=\"#cleanId\"  value=\""+(line.cleanId||0)+"\">" + 
...
function resetSum(){
	...
	jQuery("#sumWeight").val(sumWeight);
	jQuery("#sumGrains").val(sumGrains);
	jQuery("#sumMoney").val(saleMoney);
	jQuery("#sumCost").val(sumMoney);
}
...
function saveform(state){
	...
	if(!Validator.Validate($("frm"),3))
		return false;
	var saleMoney=jQuery("#saleMoney").val()||"";
	if(!(/^[-\+]?\d+(\.\d+)?$/.test(saleMoney))&&saleMoney){
		alert("加工费只能为数字");
		return ;
	}
	...
	confirm(msg, function(){
		var headObj = {
				...
				saleMoney:saleMoney?new Number(saleMoney):"",
				...
				purBillNo:jQuery("#purBillNo").val()
			};
		var lineList = [];
		for(var i=0;i<$("tbl").rows.length;i++){
			lineList.push({
...
function selectMaterialType() {
	var _iframeId = "selectMaterialTypeFrm";
	var options = {
		title : '选择发料类型',
		contentType : 'iframe',
		iframeId : _iframeId,
		...
		onok : function(box) {
			...
			if(results){
				window.location = "outVendor.vm?user_action=toAddEntrustout&materialType="+results;
				box.close();
			}else
				alert("请选择发料类型");
		},
	...
	var url =  'outVendor.vm?user_action=selectMaterialType';
...
function loadPurchaseByVendor(){
	var vendorId = jQuery("#vendorId").val();
	OutVendorDwr.getPurchaseByVendorId(vendorId,function(data){
		addOptions("purBillNo", data, null, null, true, true);
	});
}
