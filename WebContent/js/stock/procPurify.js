...
function saveform() {
	...
	confirm("确定要保存？", function(){
		...
		showLayer(true);
		ProcPurifyDwr.saveOrUpdatePurify(changeHead, oldLines, deleteOIds, function(msg){
			showLayer(false);
			if(msg) {
				alert(msg);
			} else {
				alert("保存成功", function(){
					window.location = "procPurify.vm";
				});
			}
		});
	});
...
function checkBill() {
	...
	confirm("确定要提交审核？", function(){
		...
		var deleteOIds = jQuery("#deleteOldIds").val();
		showLayer(true);
		ProcPurifyDwr.saveAndCheckPurify(changeHead, oldLines, deleteOIds, function(data){
			showLayer(false);
			if(data){
				alert(data);
			} else {
